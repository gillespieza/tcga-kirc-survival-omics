...
#   survival_data              (quick_survival_check.R)
#   rppa_univariable_results   (feature_selection.R)
#
# Produces:
...
get_genes_for_query <- function(query_name) {
  rnaseq_gene_set_membership |>
    dplyr::filter(.data$query == !!query_name) |>
...
for (pw in c("neutrophil_deg", "ecm_deg", "ptk")) {
  ...
  for (feat in top_rppa_features) {
    ...
    merged <- survival_data |>
      dplyr::select(
        dplyr::all_of(c("sample_id", pw_col, feat))
      ) |>
      tidyr::drop_na()
