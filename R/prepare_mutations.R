...
#
# Derives a sample-by-gene binary mutation feature table from the raw
# somatic mutation data. Restricts to a curated set of established ccRCC
# driver genes, converts the long mutation table to a wide binary matrix
# (1 = mutated, 0 = not mutated), and ensures all driver genes are
# represented as columns even if absent from the data.
#
# Requires: load_data.R to have been sourced so that mutation_data is
#           available in the global environment.
#
# Produces:
#   driver_genes      - Character vector of curated ccRCC driver gene
#                       symbols.
#   mutation_features - Sample-by-gene binary tibble. One row per
#                       tumour sample (sample_id) and one column per
#                       driver gene (prefixed with mut_), ordered to
#                       match driver_genes.
#
...
#
# Usage: this script is intended to be sourced by run_analysis.R as part
#        of the full pipeline, not run directly.
...
# Convert Long Mutation Table to Binary Matrix --------------------------------
...
# Warn early if any driver genes are entirely absent from the mutation
# data, before the matrix step silently drops them.
unobserved_genes <- setdiff(driver_genes, unique(mutation_long$gene_symbol))

if (length(unobserved_genes) > 0L) {
   message(
...
# Add all-zero columns for driver genes not observed in the mutation
# data, so that the feature table is complete and consistently ordered
# regardless of which genes happen to appear in this dataset.
...
# Mutation counts are reported using the shared helper from
# utils_validation.R.
