...
#
# Evaluates and compares the out-of-fold predictive performance of seven distinct
# survival models using stratified 5-fold cross-validation with fold-level 
# invariance guards to prevent statistical separation crashes:
#   1. Clinical Baseline (Age, Sex, Stage, Grade)
#   2. Clinical + Mutations
#   3. Clinical + CNA
#   4. Clinical + RPPA Proteomics (Top 5 LASSO-retained markers)
#   5. Clinical + RNA Pathways (All 8 independent curated MSigDB signatures)
#   6. Clinical + RNA Data-Driven Score (Top 20 univariable p-value genes)
#   7. Unified Multi-Omics Integrated Model (Clinical + Mutations + CNA + RPPA + RNA Pathways)
#
...
# Dynamically extract available multi-omics predictors using clean column prefixes
...
# Pull all 8 curated individual PC1 pathway scores (excluding means and data-driven scores)
rna_vars            <- names(survival_data)[
   stringr::str_starts(names(survival_data), "score_") & 
...
message("RNA Curated Pathways   : ", length(rna_vars), " (8 independent signatures)")
message("RNA Data-Driven Score  : ", length(rna_datadriven_vars), " (Top 20 univariable p-value axis)")
message("====================================")
...
# Subset to complete cases across all variables to ensure a fair, identical comparison base
...
n_folds <- 5L
# Stratify folds by event status to ensure equal distribution of deaths across splits
cv_data <- cv_data |>
   dplyr::group_by(.data$os_event) |>
   dplyr::mutate(
      fold = sample(rep(1:n_folds, length.out = dplyr::n()))
   ) |>
   dplyr::ungroup()
...
for (model_name in names(model_specs)) {
   predictions_df[[paste0("lp_", model_name)]] <- NA_real|
}
...
for (f in seq_len(n_folds)) {
   ...
   for (model_name in names(model_specs)) {
      ...
      # INVARIANCE GUARD: Filter out any binary variables that lack variation or events inside this specific fold training split
      stable_vars <- c()
      for (v in vars) {
         if (v %in% c("age", "sex", "stage", "grade") || (is.numeric(train_fold[[v]]) && !all(train_fold[[v]] == train_fold[[v]][1]))) {
            
            # Additional validation check for binary markers: ensure there are at least 2 events in each group to prevent quasi-separation
            if (!v %in% c("age", "sex", "stage", "grade") && all(range(train_fold[[v]], na.rm = TRUE) == c(0, 1))) {
               events_in_mutated  <- sum(train_fold$os_event[train_fold[[v]] == 1L], na.rm = TRUE)
               events_in_wildtype <- sum(train_fold$os_event[train_fold[[v]] == 0L], na.rm = TRUE)
               if (events_in_mutated < 2L || events_in_wildtype < 2L) {
                  next # Dynamically drop feature for this specific fold iteration to ensure stability
               }
            }
      ...
      # Fit Cox model with Efron tie handling
      fit_cv <- tryCatch(
      ...
      if (!is.null(fit_cv)) {
         # Predict risk scores (linear predictors) onto the independent held-out test fold
...
for (model_name in names(model_specs)) {
   ...
   if (all(is.na(predictions_df[[lp_col]]))) {
      warning("No valid out-of-fold predictions found for model: ", model_name, call. = FALSE)
      next
   }
...
# 6. Fit Final Integrated Model on Full Cohort -------------------------------
# Trains the ultimate unified model on the full dataset to extract multi-omics coefficients
...
message("Survival modelling complete. Model objects held in memory and exported to /results.")# Cross-validated multi-omics survival modelling ------------------------------
#
# Evaluates and compares the out-of-fold predictive performance of seven distinct
# survival models using stratified 5-fold cross-validation with fold-level 
# invariance guards to prevent statistical separation crashes:
