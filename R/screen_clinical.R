...
# Figure 1: Overview of all screened variables ---------------------------------
# Dot plot of -log10(BH-adjusted p-value) for every screened variable, ordered
# by significance. Significant variables are highlighted. A dashed line marks
# the BH threshold (p_adjust_bh = 0.05 → -log10 = 1.3).
...
# Figure 2: Forest plot of selected variables ----------------------------------
# Shows per-term hazard ratios and 95% CIs for variables that passed BH
# correction, on a log scale. Reference categories are omitted (HR = 1 by
# definition). A vertical line marks HR = 1.
...
forest_data <- clinical_univariable_detail |>
  ...
  dplyr::mutate(
    ...
    # Build a readable label combining variable and level
...
if (nrow(forest_data) > 0L) {
  ...
  message("Saved selected clinical features forest plot.")
} else {
  message("No terms available for forest plot (no significant variables).")
}
