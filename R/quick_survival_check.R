...
cohort_rna <- clinical_survival |>
   dplyr::inner_join(rnaseq_all_expression, by = "sample_id")

gene_names <- names(cohort_rna)[stringr::str_starts(names(cohort_rna), "rna_")]
...
message("\nThis may take some time. You may want to take a \u2615 break.")
message(plain_line)
...
for (i in seq_along(gene_names)) {
   gene        <- gene_names[i]
   formula_str <- paste0("survival::Surv(os_months, os_event) ~ `", gene, "`")
   form        <- stats::as.formula(formula_str)
   
   # Defensive wrapper to eliminate single-gene separation and likelihood convergence warnings
   fit <- tryCatch(
      suppressWarnings(survival::coxph(form, data = cohort_rna, ties = "efron")),
   ...
   if (!is.null(fit)) {
      ...
      if (nrow(coef_summary) > 0L) {
         rna_pvalues[i] <- coef_summary$p.value[1]
      } else {
         rna_pvalues[i] <- NA_real_
      }
   } else {
      rna_pvalues[i] <- NA_real_
   }
