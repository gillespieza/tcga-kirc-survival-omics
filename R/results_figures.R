...
# Highlight the best-performing model dynamically rather than hardcoding
best_model <- cv_results |>
  dplyr::slice_max(.data$cv_concordance, n = 1L) |>
...
plot_coefs_df <- final_coefs |>
  dplyr::mutate(
    ...
    # Data-driven score handled explicitly before the generic score_ catch-all.
    clean_feature = .data$feature,
    # Strip rna_ prefix from data-driven gene columns for readable labels
    clean_feature = stringr::str_replace(
      .data$clean_feature, "^rna_", "Gene: "
    ),
    clean_feature = stringr::str_replace(
    ...
    # Explicit label for the data-driven score before the generic replacement
    clean_feature = stringr::str_replace(
      .data$clean_feature,
      "^score_rna_datadriven$",
      "RNA Score: Data-Driven (PC1)"
    ),
    # Generic label for all remaining curated pathway scores
    clean_feature = stringr::str_replace(
      .data$clean_feature, "^score_", "RNA Score: "
    )
  ) |>
  # Assign each feature to its biological data layer for faceting.
  # Data-driven score is grouped with curated pathways under a unified panel.
  dplyr::mutate(
    layer_group = dplyr::case_when(
      ...
      .data$feature %in% c("age", "tbl_score", "sexMale") |
        ...
        "Clinical Metadata",
      # Top 10 data-driven genes entered as individual expression features
      stringr::str_starts(.data$feature, "rna_") ~
        "Data-Driven Genes (Top 10)",
      TRUE ~ "Proteomic Markers"
    ),
    layer_group = factor(.data$layer_group, levels = c(
      "Clinical Metadata", "Somatic Mutations", "Copy Number Events",
      "Proteomic Markers", "Data-Driven Genes (Top 10)",
      "Transcriptomic Pathways (8)"
    ))
  ) |>
...
save_pipeline_plot(
  ...
  width       = 1200L,
  height      = 950L,  # slightly taller to accommodate 9 pathway rows
  resolution  = 100L
)
