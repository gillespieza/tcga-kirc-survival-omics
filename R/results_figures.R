...
#
# Requires: survival_models.R to have been sourced, with results tables
#            available on disk inside the /results directory.
#
# Outputs:
...
# Calculate the exact scalar value of the clinical benchmark outside of the
# ggplot aesthetics block
...
cindex_comparison_plot <- plot_cv_df |>
  ...
  # Add shaded background banner to denote the clinical benchmark region
  ggplot2::geom_rect(
  ...
  ggplot2::labs(
    ...
    subtitle = paste(
      "True out-of-fold Harrell's C-index evaluated via stratified",
      "5-fold CV"
    ),
...
plot_coefs_df <- final_coefs |>
  dplyr::mutate(
    # Uniformly cap extreme values and wide confidence intervals
    ...
    clean_feature = stringr::str_replace(
      .data$clean_feature, "^stageSTAGE ", "Tumor Stage: "
    ),
    clean_feature = stringr::str_replace(
  ...
  # Classify features into explicit biological groups to structure vertically
  dplyr::mutate(
  ...
  # Enforce sorting inside each layer by absolute hazard risk magnitude
...
# Separate continuous features vs binary features for clinical representation
forest_plot <- plot_coefs_df |>
  ...
  ggplot2::labs(
    ...
    subtitle = paste(
      "Full-cohort Cox proportional hazards fit; elements uniformly",
      "bounded to window limits"
    ),
    x        = "Model Feature",
...
# Export high-resolution tall image to give ample vertical breathing room
save_pipeline_plot(
